...
impl Rx2xSession {
    ...
    fn estimate_drift_from_raw(&mut self) {
    ...
    fn emit_finalised(&mut self, events: &mut Vec<Rx2xEvent>) {
        ...
        // Final drift estimate from validated PLHEADER positions (LS
        // slope fit, ≥ 3 SOFs required). Used by the loop-by-loop
        // validation harness to plot injected vs estimated drift.
        self.result.final_drift_ppm = estimate_drift_from_sof_positions(
            &self.result.validated_sof_positions,
            self.cycle_period_sym,
        );
