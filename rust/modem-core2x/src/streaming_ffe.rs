...
//! into `out_buf`. When the FSM validates a PLHEADER, it calls
//! [`train_at`](StreamingFfe::train_at) with the cycle's reference
//! symbols (PLHEADER + LMS warmup + optional pre-burst). That LS-trains
//! fresh taps from `raw_buf` and re-equalises the cycle window in
//! `out_buf` with the new taps.
//!
//! **Why this exists** (cf. [[ffe-pls-chicken-and-egg]],
//! [[streaming-ffe-refactor-plan]]): the pre-refactor per-chunk
...
impl StreamingFfe {
    /// Construct a fresh block. `cycle_period` is the full PLHEADER-to-
    /// PLHEADER spacing in symbols, `training_len` is the worst-case
    /// number of reference symbols an FFE-train pass needs to see
    /// behind the SOF (= preburst + PLHEADER + LMS warmup).
    ...
    /// pass-through. Used by `reset_for_backward_fix` and
    /// `full_rebootstrap`.
    ...
    pub fn push_raw(&mut self, new_raw: &[Complex64]) {
        ...
        // Setting `RX2X_FFE_NO_FORWARD=1` makes `push_raw` pass-through
        // — needed to diagnose drift-induced LTI-violation regressions
        // where forward-applying yesterday's taps actively worsens
        // today's PLHEADER (cycle 1 at +60 ppm: gain.norm 0.56 → 0.39
        // and pls_rms 1.01 → 2.13).
        let no_forward = std::env::var_os("RX2X_FFE_NO_FORWARD").is_some();
        ...
        // Without this, every push leaks `half_taps` raw symbols into
        // `out_buf` forever — at chunk=2400 (75 sym/push) that's ~43%
        // of sym_buffer staying raw, vs ~4% at chunk=24000 (750 sym).
        // That gap was the root cause of the chunk-size-dependent DATA-CW
        // decode failures past cycle 0 (cycle 1+ data symbols fell in
        // those raw-tail patches → equalisation quality cliff → LDPC
        // didn't converge at small chunks).
        if taps_for_forward.is_some() {
            ...
            for k in fix_start..fix_end {
                // Only fix positions that had insufficient right-hand
                // context at the time of their original push. With the
                // new tail appended, `k + (n_taps - half) ≤ new_raw_len`
                // may now hold; if so, recompute. equalise_at handles
                // boundary cases (still returns raw if condition still
                // fails — defensive on the very first push).
                let y = equalise_at(
                    &self.raw_buf,
                    k,
                    taps_for_forward,
    ...
    pub fn train_at(
        ...
        if sof_abs >= self.start_abs {
            ...
            let cycle_end =
                (sof_rel + cycle_period).min(self.raw_buf.len());
            apply_ffe_inplace(
                &self.raw_buf,
                &taps,
                sof_rel,
...
/// would over- or under-flow fall through to the raw input (same
/// convention as `rx_v4::apply_ffe_to_range`).
fn equalise_at(
    raw: &[Complex64],
    k: usize,
...
/// Recompute `out_buf[start..end]` from `raw_buf` with `taps`. Boundary
/// positions copy raw verbatim (matches `rx_v4::apply_ffe_to_range`).
...
mod tests {
    ...
    fn passthrough_when_no_taps() {
        ...
        let raw: Vec<Complex64> =
            (0..50).map(|i| Complex64::new(i as f64, 0.0)).collect();
        ffe.push_raw(&raw);
    ...
    fn train_at_identity_channel() {
        ...
        let raw: Vec<Complex64> = (0..n)
            ...
            .collect();
        ffe.push_raw(&raw);
        let refs: Vec<(u64, Complex64)> =
            (10..40).map(|i| (i as u64, raw[i])).collect();
        let ok = ffe.train_at(10, &refs, 30);
        assert!(ok, "train_at failed on identity channel");
        assert!(ffe.has_taps());
        // Trained range should reproduce the input (the LS solution on
        // an identity channel returns a near-delta tap vector).
        for k in 12..38 {
    ...
    fn retention_trims_from_head() {
        ...
        // Tail of out_buf must still match tail of raw (pass-through).
    ...
    fn train_at_silently_skips_when_refs_too_few() {
        ...
        let raw: Vec<Complex64> =
            (0..60).map(|i| Complex64::new(i as f64, 0.0)).collect();
        ffe.push_raw(&raw);
        // Only 4 refs vs n_taps = 8 → must refuse.
        let refs: Vec<(u64, Complex64)> =
            (10..14).map(|i| (i as u64, raw[i])).collect();
