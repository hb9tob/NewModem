//! Modem configuration and predefined profiles (V3 wire format).
//!
...
//!
//! `ConstellationType` and `LdpcRate` are re-exported from `modem-core-base`
//! so the 2x profiles can share the same enum values without duplicating the
//! definitions.

use crate::types::DATA_CENTER_HZ;

pub use modem_core_base::profile_types::{ConstellationType, LdpcRate};
