...
fn main() {
    ...
    for _ in 0..3 {
        let _ = rx_v2::rx_v3_after(&buf, &ProfileIndex::Normal.to_config(), 0, false, None, true, false, false);
    }
    ...
    for &profile in &[
        ...
        for _ in 0..N_ITERS {
            let t0 = Instant::now();
            let _ = rx_v2::rx_v3_after(&buf, &cfg, 0, false, None, true, false, false);
    ...
    println!(
        "[perf_idle] sanity : NORMAL preamble injected → probe ratio={:.1} (threshold={}), anchor={:?}",
